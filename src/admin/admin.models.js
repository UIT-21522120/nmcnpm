...
async function ThemLopHoc(data) {
    try {
        let SQLQuery = `
            INSERT INTO LOP (MaLop, TenLop, MaHocKy, MaKhoiLop) 
            VALUES (N'${data.MaLop}', N'${data.TenLop}',N'HK0${data.HocKy}', N'${data.MaKhoiLop}');

            UPDATE LOP SET MaHocKy = (
                SELECT HK.MaHocKy FROM dbo.HOCKY HK INNER JOIN dbo.NAMHOC NH ON HK.MaNam = NH.MaNam
